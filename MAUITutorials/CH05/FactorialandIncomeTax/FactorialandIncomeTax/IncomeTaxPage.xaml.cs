	...
	private void CalculateTax_Clicked(object sender, EventArgs e)
    {
		...
		if (status == "Single")
		{
			if (income <= 11925)
				...
				tax = 1192.20m + (income - 11925) * 0.12m;
			else if (income <= 103350)
				tax = 5476.98m + (income - 48475) * 0.22m;
			else if (income <= 103350)
				tax = 5476.98m + (income - 103350) * 0.24m;
			else if (income <= 197300)
				tax = 17593.08m + (income - 197300) * 0.32m;
			else if (income <= 626350)
				tax = 57097.38m + (income - 250525) * 0.35m;
			else
